...
✅ 오류 수정 완료:
1. self.current_year → datetime.now().year
2. accident_data 필드명 수정 (올바른 CSV 컬럼명 사용)
3. IntentAgent 프롬프트 고도화 (csv_info/search_only/generate_report 명확히 구분)
"""
...
async def main(message: cl.Message):
    ...
    async with cl.Step(name="🔍 의도 분석", type="tool") as step:
        ...
        # ✅ 수정: self.current_year → datetime.now().year
...
"""
    ...
    try:
        ...
        if filtered.empty:
            await cl.Message(content=f"❌ '{date_str}' 날짜에 사고 기록이 없습니다.").send()
            return
    ...
    if len(filtered) > 1:
        if accident_number is not None and 1 <= accident_number <= len(filtered):
            ...
            await cl.Message(content=f"✅ **[{accident_number}]번 사고**를 자동 선택했습니다.").send()
        else:
            actions = []
            ...
            await cl.Message(content=cards_text + "\n**처리할 사고를 선택해주세요:**", actions=actions).send()
            
            res = await cl.AskActionMessage(content="", actions=actions, timeout=180).send()
            ...
            if res:
                ...
                accident_data = filtered.iloc[selected_idx]
                await cl.Message(content=f"✅ **[{selected_idx + 1}]번 사고**를 선택하셨습니다.").send()
            else:
    ...
    if intent == "csv_info":
        ...
        await cl.Message(content="**💬 추가 작업을 원하시나요?**", actions=actions).send()
        
        action_response = await cl.AskActionMessage(content="", actions=actions, timeout=180).send()
        
        if action_response:
            action_value = action_response.get("payload", {}).get("action") or action_response.get("value")
    ...
    if intent in ["search_only", "generate_report"]:
        ...
        await cl.Message(content=f"**📝 생성된 Query:**\n```\n{user_query}\n```\n\n**🎯 실행 모드**: {intent}").send()
        ...
        if intent == "search_only":
            actions = [
                cl.Action(name="gen_report", value="yes", label="📝 보고서 생성", payload={"action": "generate_report"}),
                cl.Action(name="exit", value="no", label="❌ 종료", payload={"action": "exit"})
            ]
            
            await cl.Message(content="**💬 보고서를 생성하시겠습니까?**", actions=actions).send()
            ...
            if action_response:
                ...
                if action_value in ["generate_report", "yes"]:
                    ...
                    final_state = await continue_to_report(final_state)
                    await display_results(final_state, "generate_report")


async def execute_agents(user_query: str, intent: str, accident_data=None) -> Dict[str, Any]:
    """Multi-Agent 시스템 실행"""
...
async def display_results(final_state: Dict[str, Any], intent: str):
    ...
    if intent == "search_only":
        ...
        if formatted_result:
            await cl.Message(content=f"## 🔍 검색 결과\n\n{formatted_result}").send()
        else:
            docs = final_state.get("retrieved_docs") or []
            await cl.Message(content=f"📊 검색된 문서 수: **{len(docs)}개**").send()
...
""").send()
        ...
        if docx_path and os.path.exists(docx_path):
            ...
            await cl.Message(content="## 📥 DOCX 파일 다운로드", elements=elements).send()
